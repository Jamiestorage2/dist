...
def get_available_gpu():
    """Get the next available GPU for mining"""
    for gpu in GPU_CONFIG['available_gpus']:
        if not gpu['in_use']:
            return gpu['id']
    # If all GPUs are in use, return the one with fewest processes
    if GPU_CONFIG['available_gpus']:
        return GPU_CONFIG['available_gpus'][0]['id']
